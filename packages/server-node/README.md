...
| `OPCUA_AUDIT_FILE`                      | —                                                       | Append-only file for the control audit trail, one JSON object per line, written _beside_ stderr. Created `0600`; a symlink, non-regular file, another account's file or a group/world-writable one stops the server, as does a file that cannot be opened. Reopened safely when rotated away. See [What is audited](https://github.com/IndustriAgents/OPCUA-MCP/blob/main/SECURITY.md#what-is-audited) |
