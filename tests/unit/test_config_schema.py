...
* the user-facing docs name no stale setting (the reference tables that name
  every one are generated by the same script, and checked by its `--check`);
...
@pytest.mark.parametrize("doc", USER_DOCS, ids=lambda p: str(p.relative_to(ROOT)))
